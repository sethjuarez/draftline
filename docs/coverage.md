...
| Sync hidden recovery support refs | Covered for local publish/fetch | `preflight_publish_support_refs`, `publish_support_refs`, `fetch_support_refs`, `list_support_refs(RemoteTracking)` | Create-only publication and remote-tracking fetch exist; remote retention and remote restore preflight remain future work. |
| Recover cleanup after clone/device loss | Partially covered | Local archive listing, remote-tracking support-ref listing, `restore_support_ref_as_variation` | Cross-machine discovery exists after fetch; restoring directly from remote-tracking support refs still needs a dedicated preflight. |
...
| 2 | Remote support-ref restore and retention | Support refs can be published/fetched, but remote-tracking refs are not yet restorable or expired through dedicated safe flows. | `preflight_restore_support_ref`, remote retention APIs |
| 3 | Agent CLI/tool facade | Coding agents need direct access to Draftline's safety model outside embedded Rust callers. | `draftline inspect`, `capabilities`, `preflight`, `execute`, `verify`, `recovery diagnose`, JSON result schema |
...
| 6 | Shared cleanup history replacement | Local squash does not replace shared remote history, and normal publish rejects rewritten shared history. | `preflight_replace_remote_history`, `replace_remote_history` |
| 7 | Remote variation lifecycle completion | Teammate-created variations can be listed/adopted, but deleted or renamed remote variations still need product diagnostics. | `fetch_all_variations`, prune/stale diagnostics |
...
| 9 | Partial save and shelf | Users need to save or put aside selected work without forcing all dirty files into one action. | `preflight_save_files`, `save_files`, `shelve_files`, `discard_files` |
...
| 11 | Preflight delete and squash | Users should know what will be archived before cleanup. | `preflight_delete_variation`, `preflight_squash_versions` |
